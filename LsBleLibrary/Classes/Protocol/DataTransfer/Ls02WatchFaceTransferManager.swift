//
//  LsCloudWatchFaceManager.swift
//  RxSwiftPro
//
//  Created by guotonglin on 2021/3/31.
...
public class Ls02WatchFaceTransferManager {
    ...
    public func requestCloudWatchFaceTransfer() {
        BleOperator.shared.requestCloudWatchFaceTransfer()
    ...
    public func cloudWatchFaceValueObserver() {
        guard let obser = BleOperator.shared.dataObserver02 else {
            return
        }
        obser.subscribe { [weak self] (p) in
            switch p {
            case let (dataType, writeMaxValue) as (Ls02DeviceUploadDataType, Int):
                if dataType == .u6101maxvalue {
                    self?.writeMaxValue = writeMaxValue - 3
                }
                print("MTU(最大发送值): \(String(describing: self?.writeMaxValue))")
            case let (dataType, orders) as (Ls02DeviceUploadDataType, (Int, Int)):
                if dataType == .cloudwatchface {
        ...
        .disposed(by: self.bag)
        // 查看6101可发送的最大值
        BleOperator.shared.readValue(type: 6101)
    }
    
    public func transferControl(_ processCode: Int, _ resumeSerialNo: Int?) {
        switch processCode {
        case 4:
    ...
    public func writeComplete() {
        BleOperator.shared.writeComplete()
            .subscribe { (result) in
                print("升级完成")
    ...
    public func writeBinData() {
        ...
        let writeCmd: [UInt8] = [0x1B, serialByte1, serialByte2]
        ...
        if(totalPackageNumber == self.currentSendIndex) {
            ...
            BleOperator.shared.directWrite(writeData, 0)
    ...
    //MARK: 4K切割数据
    public func splitData(data: Data) {
