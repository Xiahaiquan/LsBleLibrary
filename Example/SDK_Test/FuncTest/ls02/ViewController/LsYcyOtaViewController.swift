...
class LsYcyOtaViewController: UIViewController, Storyboardable {
    ...
    @IBAction func clickStartBtn(_ sender: UIButton) {
        ...
        // 从服务器下载的资源路径
        guard let binPath = Bundle.main.path(forResource: "Ls09AOTA", ofType: "bin") else {
            print("资源路径有误")
            return
        }
        
        
        self.binTransferManager = LsBinTransferManger(cbPeripheral: (BleFacade.shared.bleDevice?.peripheral!)!, binDataPath: binPath, protocalType: .LS)
        
        self.binTransferManager.start()
            .subscribe { [unowned self](progressState) in
